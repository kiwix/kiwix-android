...
class ExternalZimIntentHandlerTest {
  ...
  fun setUp() {
    ...
    coEvery { FileUtils.getLocalFilePathByUri(any(), any(), any()) } returns "/storage/test.zim"
  ...
  fun `isValidZim returns true when ZIM file is valid`() = runTest {
    coEvery { anyConstructed<ZimReaderSource>().canOpenInLibkiwix(any()) } returns true
  ...
  fun `isValidZim returns false when cannot open in libkiwix`() = runTest {
    coEvery { anyConstructed<ZimReaderSource>().canOpenInLibkiwix(any()) } returns false
  ...
  fun `handleIntent with invalid ZIM shows toast and clears intent`() = runTest {
    ...
    verify(exactly = 0) { activity.openZimFromFilePath(any()) }
  ...
  fun `handleIntent with valid ZIM opens reader and imports ZIM for playstore`() = runTest {
    ...
    verify { activity.openZimFromFilePath("content://test.zim") }
    verify { processSelectedZimFilesForPlayStore.init(any(), any(), any(), any()) }
