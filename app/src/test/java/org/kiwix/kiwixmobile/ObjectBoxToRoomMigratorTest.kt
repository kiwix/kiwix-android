...
class ObjectBoxToRoomMigratorTest {
  ...
  fun setup() {
    ...
    kiwixRoomDatabase =
      Room.inMemoryDatabaseBuilder(context, KiwixRoomDatabase::class.java)
        ...
        .build()
    val testDir = File(context.filesDir, "objectbox-test")
  ...
  @Test
  fun migrateRecentSearch_shouldInsertDataIntoRoomDatabase() =
    runBlocking {
  ...
  fun migrateHistory_shouldInsertDataIntoRoomDatabase() =
    runBlocking {
      ...
      val existingItem =
        actualData.find {
          it.historyUrl == historyItem3.historyUrl && it.historyTitle == historyItem3.title
        }
      assertNotNull(existingItem)
      val newItem =
        actualData.find {
  ...
  @Test
  fun migrateNotes_shouldInsertDataIntoRoomDatabase() =
    runBlocking {
      ...
      objectBoxToRoomMigrator.migrateNotes(box)
      notesList = kiwixRoomDatabase.notesRoomDao().notes().first() as List<NoteListItem>
      assertEquals(noteItem.title, notesList.last().title)
