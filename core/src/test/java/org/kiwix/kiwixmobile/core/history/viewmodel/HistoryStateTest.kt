...
import org.kiwix.kiwixmobile.core.history.viewmodel.effects.historyItem
import org.kiwix.kiwixmobile.core.history.viewmodel.effects.historyState
...
internal class HistoryStateTest {
  ...
  internal fun `isInSelectionMode is true when item is selected`() {
    assertThat(
      ...
      ).isInSelectionState).isEqualTo(true)
  }
  ...
  internal fun `isInSelectionMode is false when no item is selected`() {
    assertThat(
      ...
      ).isInSelectionState).isEqualTo(false)
  }
  ...
  internal fun `historyListItems should not merge dates if on different days`() {
    val item1 =
      historyItem(dateString = "today")
    val item2 =
      historyItem(dateString = "tomorrow")
    assertThat(
      historyState(
        listOf(
          item1,
