...
import org.kiwix.kiwixmobile.core.reader.ZimReader.Companion.CONTENT_PREFIX
...
class ZimReader constructor(
  val zimSource: ZimSource,
  private val jniKiwixReader: JNIKiwixReader = zimSource.createJniReader(),
  private val nightModeConfig: NightModeConfig
) {
  interface Factory {
    fun create(zimSource: ZimSource): ZimReader?

    class Impl @Inject constructor(private val nightModeConfig: NightModeConfig) :
      Factory {
      override fun create(zimSource: ZimSource) =
        try {
          ZimReader(zimSource, nightModeConfig = nightModeConfig)
  ...
  fun toBook() = Book().apply {
    title = this@ZimReader.title
    id = this@ZimReader.id
    size = "$fileSize"
    favicon = this@ZimReader.favicon
    creator = this@ZimReader.creator
    publisher = this@ZimReader.publisher
    date = this@ZimReader.date
    description = this@ZimReader.description
    language = this@ZimReader.language
    articleCount = this@ZimReader.articleCount.toString()
    mediaCount = this@ZimReader.mediaCount.toString()
    bookName = this@ZimReader.name
    tags = this@ZimReader.tags
  }
